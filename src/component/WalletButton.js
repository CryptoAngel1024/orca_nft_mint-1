...
const WalletButton = (props) => {
  ...
  const ogMint = async () => {
    ...
    const nftContract = new ethers.Contract(`${process.env.REACT_APP_NFT}`, mintABI, signer);
    try {
      await nftContract.mintOG(BigNumber.from(props.amount), { value: ethers.utils.parseEther(props.price.toString()) });
      toast.success("Successful!!!");
    } catch (err) {
      console.log("err", err["reason"])
      toast.error(err["reason"]);
    ...
    <>
      {!active ? (
        ...
        <button className="rounded-full flex items-center py-2 px-12 bg-blue-450 hover:bg-blue-460 m-0.5 space-x-2" onClick={() => mint(props.mintType)}>
          {props.mintType === 1 && <div>OG Mint</div>}
          {props.mintType === 2 && <div>Whitelist Mint</div>}
          {props.mintType === 3 && <div>Public Mint</div>}
        </button>
      )}
